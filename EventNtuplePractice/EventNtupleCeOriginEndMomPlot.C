...
bool DO_ELECTRON_END_MOMENTUM = false;
bool DO_ELECTRON_END_MOMENTUM_AFTER_700 = true;
...
void EventNtupleCeOriginEndMomPlot()
{
  ...
  if (DO_ELECTRON_END_MOMENTUM_AFTER_700==true)
  {
    TChain* ntuple = new TChain("EventNtuple/ntuple");
    ...
    TH1D* hist = new TH1D("hist","",170,30,200);
    ntuple->Draw("trkmcsim.endmom.R()>>hist","trkmcsim.time>=700","HIST");
    ...
    c1->Update();
    cout << "Saving canvas as endCeMomentumAfter700.pdf" << endl;
    c1->SaveAs("endCeMomentumAfter700.pdf");
