...
bool DO_POSITRON_MOMENTUM = true;
...
void TrkAnaTutorialPlotTrkMomentum()
{
  ...
  if (DO_POSITRON_MOMENTUM==true)
  {
    ...
    TH1D* hist3= new TH1D("hist3","",100,100,110);
    trkana->Draw("demfit.mom.R()>>hist3","demfit.sid==0 && demlh.t0>=700", "HIST SAME"); //utilizing R() to make cuts at the entrance of the tracker and also at the time greater than 700 (ns)
    hist2->SetLineColor(kGreen);
